...
//Rare exceptions to indicate a bug in the _uidining code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_uidining = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_uidining]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.uidining.module"
